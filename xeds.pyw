...
def exportCArray():
    ...
    stream = BitStream(newInstanceRoot)
    count = 0
    byteStream = stream.tobytes()
    file.write("/**"
               "\n * XEDS byte array generated by XEDS Utility " + str(version) +
               "\n * size: " + str(len(byteStream)) + " bytes\n**/\n")
    file.write("char xedsArray[] = {\n\t")
    for byte in byteStream:
        count += 1
        file.write('{0:#04x}'.format(byte))
        if count != len(stream.tobytes()):
            file.write(', ')
        if count % 8 == 0:
            file.write("\n\t")
    if count % 8 != 0:
...
fileMenu.add_command(label="Export C Array", command=exportCArray)
